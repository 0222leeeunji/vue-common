...
export default {
    ...
    regExpTest(type, value) {
        ...
        if(!regExp.test(value) || this.isNull(value)) {
            return false;
        }
    ...
    /**
     * 문자열중에 html 코드 문자열을 html로 바꾼다
     * @param type
     * @param str
     * @returns {string}
     */
    replaceHtmlCodeInStrToHtml(type, str) {
        if(this.isNull(type) || this.isNull(str))
            return "";

        switch (type) {
            case 'newLine' : return str.replace(/(\r\n|\n)/g, "<br/>");
        }
